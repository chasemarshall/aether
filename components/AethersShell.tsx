// components/AethersShell.tsx
"use client";
import { useEffect, ReactNode } from "react";

export function AethersShell({ children, theme }: { children: ReactNode; theme: "light" | "dark" | "system"; }) {
  useEffect(() => {
    if (theme === "system") {
      const mq = window.matchMedia("(prefers-color-scheme: dark)");
      document.documentElement.classList.toggle("dark", mq.matches);
      const on = (e: MediaQueryListEvent) => document.documentElement.classList.toggle("dark", e.matches);
  ...
  return (
    ...
    >
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-4">{children}</div>
    </div>
  );
}
